...
function friendRow(array $r, int $uid): array {
  return [
    'id' => (int) $r['id'], 'name' => $r['name'], 'trophies' => (int) $r['trophies'], 'avatar' => $r['avatar'] ?? null,
...
function relations(int $uid): array {
  $stmt = db()->prepare('SELECT u.id, u.name, u.trophies, u.avatar, f.status, f.requested_by
...
switch (action()) {
  case 'list': {
    $me = db()->prepare('SELECT id, name, trophies, avatar FROM users WHERE id = ?');
    $me->execute([$uid]);
    $meRow = $me->fetch();
    ok(['friends' => relations($uid), 'me' => ['id' => $uid, 'name' => $meRow['name'], 'trophies' => (int) $meRow['trophies'], 'avatar' => $meRow['avatar']]]);
  }
  ...
  case 'search': {
    ...
    $stmt = db()->prepare('SELECT id, name, trophies, avatar FROM users WHERE LOWER(name) LIKE LOWER(?) AND id <> ? ORDER BY name LIMIT 10');
    $stmt->execute([str_replace(['%', '_'], ['\\%', '\\_'], $q) . '%', $uid]);
    $known = [];
    foreach (relations($uid) as $r) $known[$r['id']] = $r['status'];
    ok(['users' => array_map(fn($r) => [
      'id' => (int) $r['id'], 'name' => $r['name'], 'trophies' => (int) $r['trophies'], 'avatar' => $r['avatar'], 'status' => $known[(int) $r['id']] ?? null,
    ], $stmt->fetchAll())]);
  }
  ...
  case 'leaderboard': {
    ...
    $me->execute([$uid]);
    $meRow = $me->fetch();
    $rows[] = ['id' => $uid, 'name' => $meRow['name'], 'trophies' => (int) $meRow['trophies'], 'avatar' => $meRow['avatar'], 'status' => 'me', 'incoming' => false];
