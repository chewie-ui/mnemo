...
4. Sur le serveur, créer `/api/.env` à partir de `.env.example` : `DB_DSN` (hôte MySQL Infomaniak, nom de base), `DB_USER`, `DB_PASSWORD`, `APP_ORIGIN=https://ton-domaine`, et `MAIL_FROM=no-reply@ton-domaine` (adresse du domaine hébergé, pour les e-mails « mot de passe oublié » envoyés via `mail()`). Si le site n’est pas à la racine du domaine, ajouter `APP_URL=https://ton-domaine/sous-dossier`. En local, aucun e-mail n’est envoyé : le lien est écrit dans `api/data/mail.log` et proposé directement dans la fenêtre.
